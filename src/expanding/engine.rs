...
impl<'a> ExpandingEngine<'a> {
    ...
    pub(crate) fn process_expanding(
        ...
        self.process_remainder(values, rem_start, global_idx, state);
    ...
    fn process_simd_chunks(
        ...
        for (c_idx, i) in chunks.enumerate() {
            let offset = (global_start_idx + c_idx * LANES) as f32;
        ...
        for (idx, &val) in values.iter().enumerate() {
            if self.compute[16] && idx < values.len() - 1 {
            ...
            if self.compute[30] {
                for (l_idx, &lag) in self.unique_c3_lags.iter().enumerate() {
                    let l = lag as usize;
                    if idx >= 2 * l {
                        let v_l = if idx - l < global_start_idx {
                            full_series[idx - l]
                        } else {
                            values[idx - l - global_start_idx]
                        };
                        let v_2l = if idx - 2 * l < global_start_idx {
                            full_series[idx - 2 * l]
                        } else {
                            values[idx - 2 * l - global_start_idx]
                        };
        ...
        if self.compute[37] {
            if self.compute[6] || self.compute[10] {
                if running_sorted.len() < full_series.len() {
                    ...
                    running_sorted.sort_unstable_by(|a, b| {
            ...
            let mut counts: SmallVec<[usize; ENTROPY_BINS]> = smallvec![0usize; ENTROPY_BINS];
            ...
            for val in full_series.iter().skip(rem_start) {
                let diff = val - mean;
                if self.compute[2] {
                    let d2 = diff * diff;
                    m2 += d2;
        ...
        self.features
            ...
            .map(|feat| match feat {
                ...
                Feature::Paa(total, index) => {
                    ...
                    if start < end {
                        let sum: f32 = full_series[start..end].iter().sum();
